...
def _validate_chamfer_reduction_inputs(
    ...
    if batch_reduction is not None and point_reduction not in ["mean", "sum"]:
        raise ValueError('point_reduction must be one of ["mean", "sum"] if batch_reduction is not None')

    # if batch_reduction is None and point_reduction == "sum":
    #     raise NotImplementedError('point_reduction sum is actually not computed when batch_reduction is None.')
    # if batch_reduction is None and point_reduction == "mean":
    #     raise NotImplementedError('point_reduction mean is actually not computed when batch_reduction is None.')
...
def chamfer_distance(
    ...
    if padded:  # Overwrite y_lengths to handle custom padding of [-100, -100, -100] points
        ridx, cidx = torch.where(y[:,:,0] == -100)  # TO FIX: check all dimensions to be -100
        if len(ridx) != 0 or len(cidx) != 0:  # whether there actually is any padding
            new_y_lengths = []
            for b in range(N):
                if (ridx == b).sum().is_nonzero():  # check if this sample `b` is padded
                    new_y_lengths.append(cidx[torch.argmax((ridx == b).type(torch.IntTensor))].item())
                else:
                    # no padding in this particular sample `b`
                    new_y_lengths.append(P2)
    ...
    if velocities:  # Custom chamfer that does not take into account velocities when computing min
        ...
        # Used for computing the attraction loss (see loss_handler)
        assert P1 == P2
    ...
    # Point reduction
    if point_reduction is not None and not avoid_in_sequence_collapsing:
        # point_reduction == "sum"
    ...
    if return_matching:
        # returned assigned pairs for both forward and backward chamfer terms
        # x_nn.idx.flatten(1, 2) : Tensor of dim [B, P1]
        # y_nn.idx.flatten(1, 2) : Tensor of dim [B, P2]
        return cham_dist, cham_normals, x_nn.idx.flatten(1, 2), y_nn.idx.flatten(1, 2)
    else:
