...
class GradientPenalty:
    ...
    def __call__(self, netD, real_data, fake_data):
        ...
        rand_shape = (real_data.shape[0], *[1 for i in range(real_data.ndim-1)])  # (B, 1, 1) for point-clouds, (B, 1) for single vectors
        alpha = torch.rand(*rand_shape, requires_grad=True).to(self.device)
